...
import { formatUsd, humanizeStatus } from "@/data/oms";
...
export default function RefundsPage() {
  ...
  const rows = orders.flatMap((o) =>
    o.refunds.map((r) => ({ order: o, refund: r })),
  );

  return (
    <div className="space-y-5">
      <div>
        <h1 className="font-headline text-2xl tracking-tight">Refunds</h1>
        <p className="mt-1 text-sm text-text/50">{rows.length} refund(s)</p>
      </div>
              ...
              >
                <div className="min-w-0">
                  ...
                  <p className="truncate text-xs text-text/50">
                    {humanizeStatus(refund.type)} refund · {order.customer.name}
