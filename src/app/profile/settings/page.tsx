...
import { User, Settings, Truck, FlaskConical, Package, ChevronRight, ChevronLeft, ChevronDown } from "lucide-react";
...
export default function ProfileSettingsPage() {
  ...
  return (
    ...
    >
      ...
      <div>
        <h1 className="font-headline text-3xl tracking-tight text-text">Settings</h1>
        <p className="mt-1 text-sm text-text/50">Account, preferences, and shipping</p>
      </div>
      ...
      <section className="rounded-2xl border border-divider/50 bg-surface/30 p-6">
        ...
        <button
          ...
          className="mt-3 flex items-center gap-1 text-sm font-medium text-accent transition-colors hover:text-accent/80"
        >
          {prefsExpanded ? "Show less" : "More options"}
          <ChevronDown
            className={`h-4 w-4 transition-transform ${prefsExpanded ? "rotate-180" : ""}`}
          />
        </button>
      </section>
