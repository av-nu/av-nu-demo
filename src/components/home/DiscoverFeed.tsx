      ...
      <div className="columns-2 gap-3 md:columns-3 lg:columns-4">
        {mixed.map((item) => {
          ...
          if (item.kind === "video") {
            ...
            return <Link key={`video-${item.id}`} href={`/post/video/${item.id}`} className="group mb-5 block w-full break-inside-avoid overflow-hidden rounded-2xl border border-divider bg-white"><div className={`relative bg-text ${item.index % 2 ? "aspect-[4/5]" : "aspect-[3/5]"}`}><video src={item.data.videoUrl} muted loop playsInline autoPlay className="h-full w-full object-cover" /></div><div className="p-3"><div className="flex items-center gap-2"><span className={`flex h-7 w-7 shrink-0 items-center justify-center rounded-full text-[9px] font-semibold text-white ${author.color}`}>{author.initials}</span><span className="truncate text-[11px] font-semibold text-text/70">@{author.handle}</span><Film className="ml-auto h-4 w-4 shrink-0 text-burgundy/60" /></div><p className="mt-3 font-headline text-base leading-tight text-text">{item.data.featured.name}</p><p className="mt-1 text-[10px] text-text/50">Watch and shop the look</p></div></Link>;
          }
