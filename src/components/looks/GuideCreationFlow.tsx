...
export function GuideCreationFlow({ lists, onComplete }: GuideCreationFlowProps) {
    ...
    <div className="border-b border-divider/60 px-4 py-4 sm:px-6">
      <div className="flex items-center justify-between gap-3">
        <div className="flex items-center gap-2">
          ...
          <div>
            <p className="text-[10px] font-semibold uppercase tracking-[0.18em] text-ember">Create a Guide</p>
      ...
      <div className="mt-4 grid grid-cols-3 gap-1.5" aria-label="Guide creation progress">
        {[1, 2, 3].map((item) => <span key={item} className={`h-1 rounded-full ${item <= stepNumber ? "bg-ember/50" : "bg-divider/70"}`} />)}
      </div>
    </div>
  );
  ...
  const renderSourceStep = () => (
    <div className="p-4 sm:p-6">
      <div className="max-w-xl">
        <span className="inline-flex items-center gap-1.5 rounded-full bg-burgundy/10 px-3 py-1 text-xs font-semibold text-burgundy"><Sparkles className="h-3.5 w-3.5" />A more considered way to create</span>
      ...
      <div className="mt-7 grid gap-3 sm:grid-cols-2">
        <button type="button" onClick={() => chooseSource("faves")} className="group rounded-2xl border border-divider/70 bg-bg p-4 text-left transition-colors hover:border-burgundy/50 hover:bg-burgundy/5 sm:p-5">
          <span className="flex h-11 w-11 items-center justify-center rounded-xl bg-burgundy/10 text-burgundy"><Heart className="h-5 w-5" /></span>
          ...
          <span className="mt-5 inline-flex items-center gap-1 text-xs font-semibold text-burgundy">Start with saved pieces <ArrowRight className="h-3.5 w-3.5 transition-transform group-hover:translate-x-1" /></span>
        </button>
        <button type="button" onClick={() => chooseSource("prompt")} className="group rounded-2xl border border-divider/70 bg-bg p-4 text-left transition-colors hover:border-ember/50 hover:bg-ember/5 sm:p-5">
          <span className="flex h-11 w-11 items-center justify-center rounded-xl bg-ember/10 text-ember"><Sparkles className="h-5 w-5" /></span>
          ...
          <span className="mt-5 inline-flex items-center gap-1 text-xs font-semibold text-ember">Find a direction <ArrowRight className="h-3.5 w-3.5 transition-transform group-hover:translate-x-1" /></span>
        </button>
      </div>
    </div>
  );
  ...
  const renderFormatStep = () => (
    <div className="p-4 sm:p-6">
      <div className="max-w-xl">
        <p className="text-xs font-semibold uppercase tracking-[0.16em] text-ember">{source === "faves" ? "My Faves" : "Prompt"}</p>
  ...
  const renderSelectedRail = () => (
    <aside className="sticky top-3 flex max-h-[calc(100vh-11rem)] min-h-28 flex-col items-center rounded-2xl border border-divider/60 bg-surface/40 p-2 sm:p-2.5">
      <div className="flex w-full flex-col items-center border-b border-divider/60 pb-2"><span className="flex h-8 w-8 items-center justify-center rounded-full bg-ember/10 text-ember"><Check className="h-4 w-4" /></span><span className="mt-1 text-center text-[9px] font-semibold uppercase tracking-[0.1em] text-text/45">Added</span><span className="text-sm font-semibold text-text">{selectedIds.length}</span></div>
  ...
  const renderProductCard = (product: NonNullable<ReturnType<typeof productFor>>, inMoreOptions = false) => {
    ...
    return <button key={product.id} type="button" onClick={() => toggleProduct(product.id)} aria-pressed={selected} className={`group relative ${inMoreOptions ? "w-full" : "w-[132px] shrink-0 sm:w-[156px]"} overflow-hidden rounded-2xl border bg-bg text-left transition-all ${selected ? "border-ember ring-2 ring-ember/20" : "border-divider/70 hover:border-ember/40"}`}>
      <div className={`relative overflow-hidden bg-surface ${inMoreOptions ? "h-52 sm:h-64" : "aspect-[4/5]"}`}><Image src={product.images[0]} alt={product.name} fill sizes={inMoreOptions ? "(max-width: 640px) 42vw, 280px" : "156px"} className="object-cover transition-transform duration-300 group-hover:scale-105" /><span className={`absolute right-2 top-2 flex h-7 w-7 items-center justify-center rounded-full shadow-sm ${selected ? "bg-ember text-white" : "bg-bg/90 text-text/45"}`}>{selected ? <Check className="h-3.5 w-3.5" strokeWidth={3} /> : <Plus className="h-3.5 w-3.5" />}</span></div>
  ...
  const renderFavesPicker = () => (
    <div className="space-y-5">
      <div><p className="text-xs leading-relaxed text-text/55">Pick a list to browse. Your added pieces stay with you if you switch lists.</p><div className="mt-3 flex gap-2 overflow-x-auto pb-1">{lists.map((list) => <button key={list.id} type="button" onClick={() => setActiveListId(list.id)} aria-pressed={activeListId === list.id} className={`flex min-w-36 shrink-0 items-center gap-2 rounded-xl border px-3 py-2 text-left ${activeListId === list.id ? "border-burgundy bg-burgundy/10 text-burgundy" : "border-divider/70 bg-bg text-text/60 hover:border-burgundy/40"}`}><ListFilter className="h-3.5 w-3.5 shrink-0" /><span className="min-w-0"><span className="block truncate text-xs font-semibold">{list.name}</span><span className="block text-[10px] opacity-60">{productIdsForList(list).length} pieces</span></span></button>)}</div></div>
      {activeList && <p className="text-[10px] font-semibold uppercase tracking-[0.14em] text-text/40">Browsing {activeList.name}</p>}
      {products.length > 0 ? <div className="flex gap-3 overflow-x-auto pb-3 snap-x">{products.map((product) => renderProductCard(product))}</div> : <div className="rounded-2xl border border-dashed border-divider/70 px-4 py-12 text-center"><Heart className="mx-auto h-7 w-7 text-burgundy/40" /><p className="mt-3 text-sm font-semibold text-text/65">This list is empty</p><p className="mt-1 text-xs text-text/45">Choose another list above, or save products to this list first.</p></div>}
    </div>
  );

  const renderPromptPicker = () => (
    <div className="space-y-5">
      <div className="rounded-2xl border border-ember/25 bg-ember/5 p-3 sm:p-4">
        <div className="flex items-start gap-3"><span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl bg-ember/10 text-ember"><Sparkles className="h-4 w-4" /></span><div className="min-w-0 flex-1"><p className="text-sm font-semibold text-text">Describe the collection you want to browse</p><p className="mt-1 text-xs leading-relaxed text-text/55">We’ll organize matching products into scrollable style and outfit categories.</p></div></div>
        <textarea value={prompt} onChange={(event) => setPrompt(event.target.value)} rows={3} placeholder="Try: a polished summer dinner look, under $150" className="mt-4 w-full resize-none rounded-xl border border-divider/60 bg-bg px-3 py-3 text-sm text-text placeholder:text-text/35 focus:border-ember/50 focus:outline-none" />
        <div className="mt-3 flex items-center gap-2"><label className="text-xs font-medium text-text/50">Budget</label><input value={budget} onChange={(event) => setBudget(event.target.value.replace(/\D/g, ""))} inputMode="numeric" placeholder="Any" className="h-8 w-20 rounded-lg border border-divider/60 bg-bg px-2 text-xs text-text focus:border-ember/50 focus:outline-none" /><button type="button" onClick={() => fileInput.current?.click()} className="ml-auto inline-flex items-center gap-1.5 rounded-lg border border-divider/70 px-2.5 py-1.5 text-[10px] font-semibold text-text/60 hover:bg-bg"><ImagePlus className="h-3.5 w-3.5" />Inspiration</button><input ref={fileInput} type="file" accept="image/*" onChange={(event) => handleImage(event.target.files?.[0])} className="sr-only" /></div>
  ...
  const renderMoreOptionsModal = () => {
    ...
    return <div className="fixed inset-0 z-50 flex items-end justify-center bg-text/45 p-0 sm:items-center sm:p-6" onClick={() => setOpenRailId(undefined)}>
      <section role="dialog" aria-modal="true" aria-labelledby="more-options-title" onClick={(event) => event.stopPropagation()} className="flex max-h-[92vh] w-full max-w-3xl flex-col overflow-hidden rounded-t-3xl bg-bg shadow-2xl sm:rounded-3xl">
        <div className="flex items-start justify-between gap-4 border-b border-divider/60 px-4 py-4 sm:px-6"><div><p className="text-[10px] font-semibold uppercase tracking-[0.16em] text-ember">More options</p><h2 id="more-options-title" className="mt-1 font-headline text-2xl tracking-tight text-text">{openRail.displayTitle ?? openRail.title}</h2><p className="mt-1 text-xs text-text/50">Browse all {openRail.productIds.length} pieces. Your selections stay added when you close this panel.</p></div><button type="button" onClick={() => setOpenRailId(undefined)} aria-label="Close more options" className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full border border-divider/70 text-text/60 hover:bg-surface"><X className="h-4 w-4" /></button></div>
        ...
        <div className="border-t border-divider/60 bg-surface/40 px-4 py-3 text-right sm:px-6"><button type="button" onClick={() => setOpenRailId(undefined)} className="rounded-xl bg-navy px-4 py-2.5 text-sm font-semibold text-white">Done</button></div>
      </section>
    </div>;
  };
  ...
  const renderPickerStep = () => (
    <div className="p-3 sm:p-6">
      <div className="mb-4 flex items-start justify-between gap-3"><div><p className="text-xs font-semibold uppercase tracking-[0.16em] text-text/40">{formatOptions.find((option) => option.value === layout)?.label} format</p><h2 className="mt-1 font-headline text-2xl tracking-tight text-text">{source === "faves" ? "Choose the pieces for your story" : "Browse by style, then add what fits"}</h2></div><span className="shrink-0 rounded-full bg-ember/10 px-2.5 py-1 text-[10px] font-semibold text-ember">{selectedIds.length} selected</span></div>
