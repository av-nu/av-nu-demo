    ...
    <div>
      ...
      <div className="grid grid-cols-2 gap-2">
        {products.map((product, index) => (
          <article key={product.id} className={`flex min-w-0 items-center gap-2 rounded-xl border p-2 ${index === 0 ? "border-pink/50 bg-pink/5" : "border-divider/60 bg-bg"}`}>
            ...
            <div className="min-w-0 flex-1">
              ...
              <button type="button" onClick={() => feature(product.id)} disabled={index === 0} className={`mt-1 inline-flex items-center gap-1 text-[9px] font-semibold ${index === 0 ? "text-pink" : "text-text/40 hover:text-pink"}`}><Star className={`h-3 w-3 ${index === 0 ? "fill-current" : ""}`} />{index === 0 ? "Featured" : "Make featured"}</button>
            </div>
            <div className="flex shrink-0 flex-col gap-1">
              ...
              <button type="button" onClick={() => onChangeAction(productIds.filter((id) => id !== product.id))} aria-label={`Remove ${product.name}`} className="flex h-6 w-6 items-center justify-center rounded-md bg-pink/10 text-pink hover:bg-pink hover:text-white"><Trash2 className="h-3 w-3" /></button>
            </div>
          </article>
        ))}
      </div>
