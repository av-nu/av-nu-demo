...
export function EditorialBuilder({ title, design, products, onChangeAction }: EditorialBuilderProps) {
    ...
    <div className="space-y-4">
      ...
      {!selected ? (
        ...
        <>
          ...
          <div><FieldLabel>Align and arrange</FieldLabel><div className="mt-2 flex flex-wrap gap-1.5"><Button label="Center horizontally" onClick={() => alignSelected("horizontal")}><AlignHorizontalJustifyCenter className="h-3.5 w-3.5" /></Button><Button label="Center vertically" onClick={() => alignSelected("vertical")}><AlignCenter className="h-3.5 w-3.5" /></Button><Button label="Bring to front" onClick={() => reorder("front")}><BringToFront className="h-3.5 w-3.5" /></Button><Button label="Send to back" onClick={() => reorder("back")}><SendToBack className="h-3.5 w-3.5" /></Button></div></div>
  ...
  const renderAddPanel = () => (
    <div className="space-y-4">
      ...
      <div><FieldLabel>Products</FieldLabel><div className="mt-2 grid max-h-64 grid-cols-3 gap-2 overflow-y-auto pr-1">{products.map((product, index) => <button key={product.id} type="button" onClick={() => addElement(createProductElement(product.id, index))} className="group overflow-hidden rounded-xl border border-divider/60 bg-bg text-left"><span className="relative block aspect-square bg-surface"><Image src={product.images[0]} alt={product.name} fill sizes="120px" className="object-cover transition-transform group-hover:scale-105" /></span><span className="block truncate px-2 py-1.5 text-[10px] font-semibold text-text/65">{product.name}</span></button>)}</div></div>
    </div>
  );

  const renderLayers = () => (
    <div className="space-y-2">{[...present.elements].sort((a, b) => b.zIndex - a.zIndex).map((element) => <button key={element.id} type="button" onClick={() => { setSelectedId(element.id); setPanel("design"); }} className={`flex w-full items-center gap-2 rounded-xl border px-3 py-2 text-left ${selectedId === element.id ? "border-accent bg-accent/5" : "border-divider/60 bg-bg"}`}><span className="flex h-7 w-7 items-center justify-center rounded-lg bg-surface text-text/50">{element.type === "text" ? <Type className="h-3.5 w-3.5" /> : element.type === "shape" ? <Shapes className="h-3.5 w-3.5" /> : element.type === "video" ? <Film className="h-3.5 w-3.5" /> : <ImagePlus className="h-3.5 w-3.5" />}</span><span className="min-w-0 flex-1 truncate text-xs font-semibold text-text/70">{element.name}</span>{element.locked && <Lock className="h-3 w-3 text-text/35" />}{element.hidden && <EyeOff className="h-3 w-3 text-text/35" />}</button>)}</div>
  );
