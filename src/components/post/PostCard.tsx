  ...
  return (
    <article className="overflow-hidden rounded-2xl border border-divider/50 bg-bg">
      ...
      {post.productIds.length > 0 && (
        <div className="pt-3">
          <button type="button" onClick={onOpen} className="flex w-full items-center justify-center gap-1.5 bg-[#561F59] px-2.5 py-2 text-[11px] font-semibold text-white transition-colors hover:bg-[#561F59]/90 md:gap-2 md:px-3 md:py-2.5 md:text-xs">
            <ShoppingBag className="h-3.5 w-3.5 shrink-0" />
            <span className="whitespace-nowrap">Shop the post</span>
            <span className="shrink-0 rounded-full bg-white/20 px-2 py-0.5 text-[10px] font-bold">{post.productIds.length}</span>
          </button>
        </div>
      )}
