  ...
  return (
    <section>
      <div className="mb-4 flex items-center justify-between gap-3">
        ...
        <Link href="/create-a-look" className="shrink-0 text-sm font-semibold text-accent hover:underline">Create new</Link>
      </div>
      {recentLooks.length === 0 ? (
        <Link href="/create-a-look" className="flex items-center justify-between rounded-2xl border border-dashed border-divider/60 p-5 transition-colors hover:border-accent/40 hover:bg-surface/30">
          <span><span className="block text-sm font-semibold text-text">Your Lookbook will live here</span><span className="mt-1 block text-sm text-text/50">Start with a vibe and make it yours.</span></span>
        ...
        <div className="grid gap-3 sm:grid-cols-3">
          {recentLooks.map((look) => {
            ...
            return (
              <Link key={look.id} href={`/create-a-look/${look.id}`} className="group overflow-hidden rounded-2xl border border-divider/60 bg-surface/30 transition-colors hover:border-accent/40">
